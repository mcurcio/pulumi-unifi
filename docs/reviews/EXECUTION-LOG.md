...
- D-M3.3 `exclude-resources` pass: Voucher + AdoptDevice removed as resources (mappings.yaml excludeResources, data-only) preserving getAdoptedDeviceDetail/listAdoptedDeviceOverviews/getVoucher data sources. 21->19 resources. Verified unit+mock first-try-green; golden = exactly 2 resource removals, 0 function removals. *** TRACK D COMPLETE ***
